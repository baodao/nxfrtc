...
function SimpleWebRTC(opts) {
    ...
    var config = this.config = {
            //url: '202.112.146.103:8181',
	    // url: 'http://signaling.simplewebrtc.com:8888',
            url: '192.168.1.157:8888',
            debug: false,
            localVideoEl: '',
            remoteVideosEl: '',
    ...
    connection.on('onlinelist',function(payload){
       self.emit('onlinelist',payload);
    });
...
SimpleWebRTC.prototype.adduserslist =function(){
    var self=this;
    var message ={
        room:$('#roomname')[0].value,
        nickname:$('#username')[0].value,
        type:'onlinelist',
        action:'add',
        prefix: 'webkit'
        };
   self.webrtc.emit('message',message);
}
SimpleWebRTC.prototype.updateuserslist = function(){
    var self=this;
    var message ={
        room:$('#roomname')[0].value,
        nickname:$('#username')[0].value,
...
SimpleWebRTC.prototype.removeuserslist = function(){
    var self=this;
    var message ={
        room:$('#roomname')[0].value,
        nickname:$('#username')[0].value,
...
SimpleWebRTC.prototype.sendchat=function(channel,message,payload){
    var self=this;
    self.webrtc.sendDirectlyToAll(channel,message,payload);
}
...
Peer.prototype.handleMessage = function (message) {
    ...
    if (message.type === 'offer') {
       ...
       usersmap[self.id]=message.payload;
    }    
};
