...
io.sockets.on('connection', function (client) {
    ...
    client.on('message', function (details) {
	    if (details.type == 'onlinelist'){
		  if (details.action == 'add'){
		    usersWs[details.nickname]=details.room;
		  }
		  if (details.action == 'remove'){
		    delete usersWs[details.nickname];
		  }
		  if (details.action == 'update'){
		    var userslist={};
		    for (var i in usersWs){
		      if (details.room == usersWs[i]) {
			    userslist[i]=usersWs[i];
		       }
			 }
			client.emit('onlinelist',userslist);
		  }
		}
		else{
